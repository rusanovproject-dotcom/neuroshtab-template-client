...
```
✅ Архитектор офиса в команде.

Он не делает контент — у него одна задача: смотреть чтобы офис не превращался
в свалку через 3 месяца. Ревизует структуру, чистит память, ловит дубли в
знаниях, собирает повторяющиеся ошибки агентов в правила.

Старый аудит (`/audit-project`) теперь устарел — Архитектор офиса делает то же,
только строже: с цитатами на каждый файл, с обязательной секцией «что не трогать»,
с явным согласием перед любой правкой. Старый скилл переехал в архив, его триггеры
(«проверь офис», «наведи порядок», «аудит») теперь у Архитектора.

Также я создал `.claude/settings.json` (или дополнил существующий) — там список
правил, что Архитектор НЕ имеет права читать или менять: `.env`, `*.pem`,
`credentials*`, плюс заблокированы опасные команды (`rm`, `git push --force`,
`git reset --hard`). Это базовая защита офиса.

Три режима — выбираешь по ситуации:

1. **Быстрый скан** (5 минут) — смотрит структуру и пишет короткий отчёт.
   Только смотрит, ничего не трогает. Запуск: «проверь офис» или
   `/office-architect`.

2. **Лёгкая чистка** (15 минут) — скан + правит мелочи под твоё согласие.
   Frontmatter, даты, перенос устаревшего в архив. Каждое изменение
   спрашивает: да / только важное / "1,3" / нет. Запуск: «почистим офис»
   или `/office-architect tidy`.

3. **Полный аудит** (60 минут) — всё по 6 направлениям + 8-мерная рубрика
   + сводный балл 0-100 + предложения как поднять качество. Запуск:
   «полный аудит» или `/office-architect deep`.

Все отчёты ложатся в `office/ops/audits/` с датой — потом можно сравнить
прогресс через месяц.

Главное: он **никогда** не трогает `.env`, не удаляет файлы и не правит
ядро агентов без твоего явного согласия. Всё append-only — старые версии
уезжают в `_archive/`.
```
...
first_task:
  ...
  mode: scan
  safety_note: "scan = read-only, никаких изменений в файлах"
```

Сообщение CTA в конце post-install message:

> *«Давай начнём с быстрого скана? 5 минут, ничего не трогает — увидишь актуальную картину офиса. Скажи "проверь офис" и я запущу.»*
