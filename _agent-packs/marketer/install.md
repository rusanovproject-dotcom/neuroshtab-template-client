...
version: 1.1.0
requires: []
provides_pipeline: audience-unpack-kadyrov  # глубокий пайплайн распаковки ЦА для шаблона project-template/audience/segments/
```
...
updates:
  ...
  - file: office/AGENTS.md
    ...
    add_row: |
      | **Маркетолог** | Клиентская база, тест-процесс гипотез, распаковка ЦА (Кадыров pipeline), продуктовая линейка, воронка | *«распакуй мою ЦА», «найди сегменты», «провёл диагностику», «разведай конкурентов», «пятничная ревизия»* |
...
2. **Глубокая распаковка ЦА** (если стоит шаблон project-template) —
   ...
   • найдёт ТОП-3 сегмента через 6-осевой scoring (Кадыров)
