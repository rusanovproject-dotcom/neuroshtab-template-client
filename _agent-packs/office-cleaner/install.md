...
updates:
  ...
  - file: office/agents/director/knowledge/routing-patterns.md
    ...
    # В базовом шаблоне уже есть строки аудита, ведущие на `/office-cleaner` (пак ещё не установлен — маршрут с оговоркой «предложи install»).
    # При установке ЗАМЕНЯЕМ их обогащёнными (триггеры по имени «рита»). Не дублируй: если строки с «рита» уже есть — пропусти.
    replace_row:
      match_contains: "/office-cleaner"
      with_rows: |
