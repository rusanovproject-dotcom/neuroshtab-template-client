...
description: >
  Генерирует готовый промт для claude.ai/design на основе Brand Book пользователя и типа дизайна.
  Результат — 2-3 абзаца текста, готовые к копированию. Запускается когда пользователь выбрал
...
Твоя задача — превратить задачу пользователя в мега-промт для claude.ai/design. 2-3 абзаца. Пользователь копирует → вставляет → получает результат.
...
Из design-catalog.md — размер в пикселях и ratio. Если не из каталога — уточни у пользователя.

### 3. Собери концепцию

3-5 строк у себя в голове (не вываливай пользователю если не просит):
...
После промта — одно предложение для пользователя:
