...
Если клиент после `/clear` пишет «продолжим с ЦА» / «вернёмся к JTBD» — Алекс читает `agent-state.md` + `JTBD_прогресс_<main>.md`, понимает где остановились, продолжает работу с того места без переспрашивания.
