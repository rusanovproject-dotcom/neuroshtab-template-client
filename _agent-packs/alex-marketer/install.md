...
short_role: Маркетолог-Хормози. Рынок > Оффер > Копия. Распаковывает ЦА (Кадыров pipeline), собирает Grand Slam Offer (Value Equation), строит воронку. Двигает проект гипотезами (max 5 + Key). Режет хуйню, защищает сильные идеи.
...
requires: []
provides_pipeline:
  - audience-unpack-kadyrov  # глубокий пайплайн распаковки ЦА для шаблона project-template/audience/segments/
...
updates:
  ...
  - file: office/AGENTS.md
    ...
    add_row: |
      | **Алекс Маркетолог** | Маркетолог-Хормози: распаковка ЦА (Кадыров), оффера (Хормози), воронки. Тест-процесс гипотез. Клиентская база (через `/marketer-enable-meetings`) | *«распакуй проект», «распакуй ЦА», «распакуй оффер», «найди сегменты», «разведай конкурентов»* |
