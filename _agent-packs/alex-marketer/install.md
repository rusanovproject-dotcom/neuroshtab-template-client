...
files:
  ...
  - src: agent-state-template.md
    dest: office/agents/alex-marketer/agent-state-template.md
  - src: knowledge/
...
После копирования файлов — развернуть рабочее состояние агента из шаблона. Идемпотентно: если `agent-state.md` уже есть (переустановка), не затирать накопленное состояние.

```bash
[ -f office/agents/alex-marketer/agent-state.md ] || cp office/agents/alex-marketer/agent-state-template.md office/agents/alex-marketer/agent-state.md
```

Без `agent-state.md` Hard Pre-flight Алекса (см. `core.md` → Input Contract) не находит активного клиента/Stage и каждый раз создаёт состояние с нуля. Шаблон — стартовая точка с `onboarding_completed: false`.
...
updates:
  ...
  - file: office/AGENTS.md
    ...
    add_row: |
      | **Алекс Маркетолог** | Распаковка ЦА через JTBD-методологию. На выходе — `JTBD_анализ.md` с сегментами, jobStory, картой работ, барьерами. Если в офисе мало живой фактуры — опциональный модуль `/jtbd-aicustdev` собирает синтетические касдевы (🔴 заглушка для первого черновика). | *«онбординг алекс», «распакуй ЦА», «JTBD», «найди сегменты», «нет данных — синтетика»* |
  ...
  - file: office/agents/director/core.md
    ...
    add_rows: |
      ...
      | нет данных от клиентов, синтетический касдев, AI касдев, сгенерируй интервью, нет фактуры | **Алекс** → `/jtbd-aicustdev` *(опциональный модуль внутри /jtbd — не отдельная точка входа)* |
      | провёл диагностику, был созвон, клиент оплатил, клиент отказался, вот транскрипт | **Алекс** → `/jtbd-log-deal` *(требует активации модуля встреч)* |
```
...
3. **Опционально — синтетические касдевы** (`/jtbd-aicustdev`)
   Если у тебя пока мало живой фактуры от платящих клиентов (меньше 5
   источников от 3+ разных клиентов) — на старте JTBD-распаковки появится
   развилка. Можно либо подкинуть свои данные (диагностики, кейсы,
   переписки), либо я соберу 10-15 синтетических интервью за 30-40 минут.
   Все цитаты из синтетики помечаются 🔴 AI-генерация — это **временная
   заглушка** для первой версии анализа. Перед закрытием Stage 1 нужно
   заменить синтетику на 3-5 живых интервью по сегменту-фокусу.

4. **Опционально — модуль встреч** (`/marketer-enable-meetings`)
