...
requires: []
provides_pipeline:
  - audience-jtbd-pipeline  # JTBD-распаковка ЦА (13 шагов на сегмент)
```
...
# Шаблон папки распаковки ЦА — критичный, без него /audience-jtbd Шаг 2
...
project_templates:
  - src: ../../projects/_template-audience/
    ...
    description: |
      ...
      Используется в /audience-jtbd Шаг 2 как источник структуры:
...
updates:
  ...
  - file: office/agents/director/core.md
    ...
    add_rows: |
      ...
      | распакуй ЦА, найди сегменты, кто наша ЦА, JTBD, jobs to be done, работа клиента, поехали распаковывать | **Алекс** → `/audience-jtbd` *(если онбординг не пройден — авто-route на `/alex-onboarding`)* |
      | проверь JTBD, свежий взгляд на анализ, аудит JTBD, критик JTBD | **Алекс** → `/audience-jtbd-critic` *(новый чат, чистый контекст)* |
...
1. **JTBD-распаковка ЦА** (`/audience-jtbd`)
...
2. **Финальная проверка критиком** (`/audience-jtbd-critic`)
