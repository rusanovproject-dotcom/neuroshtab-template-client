#!/bin/bash
# validate-agent.sh — Проверка агента БЕЗ LLM
# Ловит 80% проблем бесплатно. Запускать ПЕРЕД Judge/Validator.
...
done < <(grep -oE '`[^`]+\.(md|sh|json)`' "$CLAUDE_MD" 2>/dev/null | sed 's/`//g' || true)
