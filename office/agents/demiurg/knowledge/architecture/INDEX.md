...
| `office-types/mixed.md` | **При сборке mixed-офиса (Нейроштаб, гибрид)** — pod-decomposition, переcечения зон, 3 namespace памяти |
...
"гибрид" / "Нейроштаб" / "разные домены вместе":
  - office-blueprint.md
...
## Конвенция ссылок-источников (НЕ править как «битые пути»)

В дистиллятах этой папки в блоках `**Источники:**` и в разборах-контрастах встречаются два префикса, указывающие на **внешние офисы времени дистилляции**, а не на локальные файлы шаблона:

- **`office/...`** — главный «живой» офис-источник (Director на 17 агентов, со всеми плюсами и минусами; часто как пример «чему НЕ повторять»).
- **`client-office-template/...`** — этот чистый шаблон как эталон-контраст («clean Director / Designer»). Папки `client-office-template/` внутри шаблона нет — это distillation-time имя.

Оба — **намеренные цитаты происхождения**, а не навигируемые ссылки. Валидатор (`validate-office.sh`, проверка C2) может пометить `client-office-template/...` как broken link — это ложный позитив, оставляем. **Не стрипать префикс** `client-office-template/` → `office/`: это схлопнет контраст «живой vs чистый» (а `office/` здесь означает другой, 17-агентный офис).
