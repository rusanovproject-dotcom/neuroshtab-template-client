...
| `edge-cases.md` | **Справочник пограничных сценариев** — нет Claude Pro, результат не зашёл, тип не из каталога, клиент работает в другом AI, Claude обрезал лендинг, хочу как у чужого сайта, и др. Читать когда ситуация нестандартная. |
...
| `brand-references-catalog.md` | **30 эталонных брендов** (Apple, Stripe, Linear, Notion, Vercel, Framer, Claude и др.) со slug'ами для `npx getdesign add {slug}`. Используется когда клиент говорит "хочу как [бренд]" — загружаем их DESIGN.md через skill `brand-reference`. |
...
- **Палитра — из ниши клиента первой опцией.** `brand-palette-guide.md` даёт "Industry Standard" для 40 ниш — используй в Brand Book онбординге как Вариант №1.
...
- **Эталонные бренды грузятся командой.** Клиент говорит "как Apple" → skill `brand-reference` → `npx getdesign add apple`. Каталог 30 брендов в `brand-references-catalog.md`.
